...
Library.Brand = {
    Title = "UI Library", SubTitle = "Free Edition", Version = "v1.0.0",
    Discord = "", AboutTitle = "About", AboutBody = "Built with Babis UI Library.", AboutEnabled = true,
}
Library.Config = {
    folder = "BabisUI/configs", ext = ".json", autoloadFile = "BabisUI/autoload.txt",
    introDuration = 5.5, notifySoundVol = 0.5,
}
...
Library.Theme = {
    colors = {
        ...
        bellBg=Color3.fromRGB(30,38,48), sectionTitle=Color3.fromRGB(160,210,255),
        cfgIcon=Color3.fromRGB(170,120,255), cfgBtnBg=Color3.fromRGB(24,24,28), cfgBtnBorder=Color3.fromRGB(48,48,56),
    },
    sizes = {
        windowWidth=460, windowHeight=700, windowRadius=22, headerHeight=82, navHeight=88,
        navBtnSize=70, navBtnRadius=14, navGap=8, navPadX=12, contentPadX=14,
        scrollPadL=6, scrollPadR=8, iconSize=46, cardRadius=16, cardPadX=16, cardGap=14, cardHeight=128,
        toggleW=72, toggleH=38, toggleKnob=30, toggleRight=22,
        controlW=130, controlH=44, controlRight=22, handIconSize=82, handIconY=20,
        sectionHeaderH=34, cfgHeight=560,
    },
    fonts = { appName=Enum.Font.GothamBold, version=Enum.Font.Gotham, intro=Enum.Font.Code },
    textSizes = {
        appName=22, version=13, intro=20, cardTitle=24, cardDesc=15,
        aboutTitle=30, aboutDesc=19, discordTitle=28, discordSub=18, sectionTitle=18,
...
Library.Assets = {
    handIcon="rbxassetid://88060480140568", infoIcon="rbxassetid://7733964719",
    discordIcon="rbxassetid://100770414662869", bellIcon="rbxassetid://7072706001",
    notifySound="rbxassetid://5153734608", introImage="rbxassetid://119251117614023",
...
function Library:SetAutoload(name)
    self:_ensureCfgFolder()
    if name == nil or name == "" then safeCall(delfile, self.Config.autoloadFile)
    else safeCall(writefile, self.Config.autoloadFile, name) end
end
function Library:SetBrand(t)
    for k, v in pairs(t or {}) do self.Brand[k] = v end
    for _, w in ipairs(self._windows or {}) do if w.ApplyBrand then w:ApplyBrand() end end
end
...
function Library:PlayIntro(cfg, onComplete)
    if type(cfg) == "function" and onComplete == nil then onComplete = cfg; cfg = {} end
    cfg = cfg or {}
    ...
    local old = PlayerGui:FindFirstChild("BabisIntro"); if old then old:Destroy() end
    local introGui = Instance.new("ScreenGui")
    introGui.Name = "BabisIntro"; introGui.ResetOnSpawn = false
    introGui.IgnoreGuiInset = true; introGui.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
    introGui.DisplayOrder = 200; introGui.Parent = PlayerGui
    ...
    glow.AnchorPoint = Vector2.new(0.5,0.5); glow.BackgroundColor3 = self.Theme.colors.accent
    glow.BackgroundTransparency = 1; glow.BorderSizePixel = 0; glow.Parent = center
    corner(glow, 110)
    ...
    image.Size = UDim2.new(0,imgW,0,imgH); image.Position = UDim2.new(0.5,0,0.5,0)
    image.AnchorPoint = Vector2.new(0.5,0.5); image.BackgroundTransparency = 1
    image.Image = img; image.ImageTransparency = 1; image.ScaleType = Enum.ScaleType.Fit
    image.Parent = center
    ...
    subtitle.Text = text; subtitle.TextColor3 = Color3.fromRGB(225,230,245)
    subtitle.TextTransparency = 1; subtitle.Font = self.Theme.fonts.intro
    subtitle.TextSize = self.Theme.textSizes.intro; subtitle.TextXAlignment = Enum.TextXAlignment.Center
    subtitle.Parent = overlay

    local line = Instance.new("Frame")
    line.Size = UDim2.new(0,0,0,1); line.Position = UDim2.new(0.5,0,0.5,158)
    line.AnchorPoint = Vector2.new(0.5,0); line.BackgroundColor3 = self.Theme.colors.accent
    ...
    task.spawn(function()
        while spinning do
            local spin = TweenService:Create(image, TweenInfo.new(1.6, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), {Rotation = image.Rotation + 360})
    ...
    task.spawn(function()
        while spinning do
            ...
            task.wait(0.9); if not spinning then break end
...
function Notifier.new(theme, assets, config)
    ...
    self.theme = theme; self.assets = assets; self.config = config; self.order = 0
    local old = PlayerGui:FindFirstChild("BabisUILibNotif"); if old then old:Destroy() end
    local gui = Instance.new("ScreenGui")
    gui.Name = "BabisUILibNotif"; gui.ResetOnSpawn = false; gui.IgnoreGuiInset = true
    gui.ZIndexBehavior = Enum.ZIndexBehavior.Sibling; gui.DisplayOrder = 300; gui.Parent = PlayerGui
    local sound = Instance.new("Sound")
    sound.SoundId = assets.notifySound; sound.Volume = config.notifySoundVol
    sound.Parent = SoundService; self.sound = sound
    ...
    container.Position = UDim2.new(1,-20,0,20); container.Parent = gui
...
function Notifier:notify(title, message, duration)
    ...
    row.ClipsDescendants = false; row.LayoutOrder = self.order; row.Parent = self.container
    ...
    corner(card, 12); stroke(card, self.theme.colors.cardBorder, 1.5, 0)
    ...
    bellBg.BackgroundColor3 = self.theme.colors.bellBg; bellBg.BorderSizePixel = 0
    bellBg.Parent = card; corner(bellBg, 21); stroke(bellBg, ACCENT, 1.2, 0.4)
    local bell = Instance.new("ImageLabel")
    bell.Size = UDim2.new(0,28,0,28); bell.Position = UDim2.new(0.5,-14,0.5,-14)
    bell.BackgroundTransparency = 1; bell.Image = self.assets.bellIcon
    ...
    titleLbl.TextYAlignment = Enum.TextYAlignment.Top; titleLbl.Size = UDim2.new(1,-90,0,22)
    titleLbl.Position = UDim2.new(0,72,0,16); titleLbl.Parent = card
    ...
    msgLbl.Size = UDim2.new(1,-90,0,34); msgLbl.Position = UDim2.new(0,72,0,38); msgLbl.Parent = card
    ...
    task.spawn(function() task.wait(duration); self:dismiss(row) end)
    return row
end
...
local function makeCard(scroll, order, height, theme)
    ...
    card.BackgroundColor3 = theme.colors.cardBg; card.BorderSizePixel = 0
    ...
    title.BackgroundTransparency = 1; title.TextColor3 = theme.colors.textPrimary
    title.Font = Enum.Font.GothamBold; title.TextSize = theme.textSizes.cardTitle
    title.TextXAlignment = Enum.TextXAlignment.Left; title.TextYAlignment = Enum.TextYAlignment.Top
    title.Size = UDim2.new(1,-160,0,30); title.Position = UDim2.new(0,theme.sizes.cardPadX,0,24)
    title.Parent = card
    local desc = Instance.new("TextLabel")
    desc.BackgroundTransparency = 1; desc.TextColor3 = theme.colors.textDesc
    desc.Font = Enum.Font.Gotham; desc.TextSize = theme.textSizes.cardDesc
    desc.TextXAlignment = Enum.TextXAlignment.Left; desc.TextYAlignment = Enum.TextYAlignment.Top
    desc.Size = UDim2.new(1,-160,0,22); desc.Position = UDim2.new(0,theme.sizes.cardPadX,0,68)
    desc.Parent = card
...
function Button.new(section, cfg)
    local window = section.tab.window; local library = window.library
    local theme = library.Theme; local scroll = section.tab.scroll
    local order = section:_nextOrder()
    local titleText = cfg.Name or "Button"; local descText = cfg.Description or ""
    local onClick = cfg.Callback; local doNotify = cfg.Notify ~= false
    local card, title, desc, cardStroke = makeCard(scroll, order, nil, theme)
    title.Text = titleText; desc.Text = descText
    local iconSize = theme.sizes.handIconSize; local iconY = theme.sizes.handIconY
    ...
    hand.BackgroundTransparency = 1; hand.Image = library.Assets.handIcon
    hand.ImageColor3 = theme.colors.handIdle; hand.ScaleType = Enum.ScaleType.Fit; hand.Parent = card
    local flash = Instance.new("Frame")
    flash.Size = UDim2.new(1,0,1,0); flash.BackgroundColor3 = theme.colors.accent
    flash.BackgroundTransparency = 1; flash.BorderSizePixel = 0; flash.ZIndex = 3
    flash.Parent = card; corner(flash, theme.sizes.cardRadius)
    ...
    local function fire()
        ...
        local pressSize = UDim2.new(baseSize.X.Scale, baseSize.X.Offset-6, baseSize.Y.Scale, baseSize.Y.Offset-6)
    ...
    self._fire = fire; hit.Activated:Connect(fire)
...
function Toggle.new(section, cfg)
    ...
    local theme = library.Theme; local scroll = section.tab.scroll
    local order = section:_nextOrder()
    local titleText = cfg.Name or "Toggle"; local descText = cfg.Description or ""
    local onChange = cfg.Callback; local doNotify = cfg.Notify ~= false
    local state = cfg.Default and true or false
    local card, title, desc, cardStroke = makeCard(scroll, order, nil, theme)
    title.Text = titleText; desc.Text = descText
    local tW, tH = theme.sizes.toggleW, theme.sizes.toggleH
    local knob = theme.sizes.toggleKnob; local pad = (tH - knob) / 2
    ...
    track2.BackgroundColor3 = theme.colors.toggleOff; track2.BorderSizePixel = 0
    ...
    knobFrame.BackgroundColor3 = theme.colors.textPrimary; knobFrame.BorderSizePixel = 0
    ...
    function self:Set(v, fire)
        ...
        self._state = v; render(true, fire ~= false, false)
    end
...
function Slider.new(section, cfg)
    ...
    local theme = library.Theme; local scroll = section.tab.scroll
    local order = section:_nextOrder()
    local titleText = cfg.Name or "Slider"; local descText = cfg.Description or ""
    local minV = cfg.Min or 0; local maxV = cfg.Max or 100
    local value = cfg.Default or minV; local onChange = cfg.Callback
    ...
    barBg.BackgroundColor3 = theme.colors.toggleOff; barBg.BorderSizePixel = 0
    barBg.Parent = card; corner(barBg, barH/2)
    local fill = Instance.new("Frame")
    fill.Size = UDim2.new(0,0,1,0); fill.BackgroundColor3 = theme.colors.accent
    ...
    knob.BackgroundColor3 = theme.colors.textPrimary; knob.BorderSizePixel = 0
    knob.ZIndex = 5; knob.Parent = barBg; corner(knob, 13); stroke(knob, theme.colors.accent, 2, 0)
    local valueLbl = Instance.new("TextLabel")
    valueLbl.BackgroundTransparency = 1; valueLbl.Text = tostring(value)
    valueLbl.TextColor3 = theme.colors.accent; valueLbl.Font = Enum.Font.GothamBold
    valueLbl.TextSize = 18; valueLbl.TextXAlignment = Enum.TextXAlignment.Right
    valueLbl.TextYAlignment = Enum.TextYAlignment.Center; valueLbl.Size = UDim2.new(0,80,0,26)
    valueLbl.Position = UDim2.new(1,-104,1,-50); valueLbl.Parent = card
    ...
    local function setRel(rel, fire)
        ...
        self._value = v; valueLbl.Text = tostring(v)
    ...
    local function setFromX(absX)
        local barAbs = barBg.AbsolutePosition.X; local barSize = barBg.AbsoluteSize.X
    ...
    function self:Destroy() connChanged:Disconnect(); connEnded:Disconnect(); card:Destroy() end
    return self
end
...
function Dropdown.new(section, cfg)
    ...
    local theme = library.Theme; local scroll = section.tab.scroll
    local order = section:_nextOrder()
    local titleText = cfg.Name or "Dropdown"; local descText = cfg.Description or ""
    local choices = cfg.Options or {}; local value = cfg.Default or choices[1]
    local onChange = cfg.Callback; local doNotify = cfg.Notify ~= false
    local card, title, desc, cardStroke = makeCard(scroll, order, nil, theme)
    title.Text = titleText; desc.Text = descText
    ...
    ddBtn.BackgroundColor3 = theme.colors.navBg; ddBtn.Text = value or "---"
    ddBtn.TextColor3 = theme.colors.textPrimary; ddBtn.Font = Enum.Font.GothamBold
    ddBtn.TextSize = 17; ddBtn.TextXAlignment = Enum.TextXAlignment.Left
    ddBtn.AutoButtonColor = false; ddBtn.Parent = card; corner(ddBtn, 10)
    local ddStroke = stroke(ddBtn, theme.colors.cardBorder, 1.5, 0)
    local padL = Instance.new("UIPadding"); padL.PaddingLeft = UDim.new(0,12); padL.Parent = ddBtn
    local arrow = Instance.new("TextLabel")
    arrow.Size = UDim2.new(0,20,0,theme.sizes.controlH); arrow.Position = UDim2.new(1,-24,0,0)
    arrow.BackgroundTransparency = 1; arrow.Text = "^"; arrow.TextColor3 = theme.colors.accent
    arrow.Font = Enum.Font.GothamBold; arrow.TextSize = 16; arrow.Rotation = 180; arrow.Parent = ddBtn
    ...
    local function openDd()
        ...
        ddListOpen = true
        local wrapper = window._scaleWrapper; local uiScale = window._uiScale
        ...
        ddBackdrop.Parent = wrapper; ddBackdrop.Activated:Connect(closeDd)
        local btnAbs = ddBtn.AbsolutePosition; local wrapAbs = wrapper.AbsolutePosition
        local s = uiScale.Scale
        local relX = (btnAbs.X - wrapAbs.X) / s; local relW = ddBtn.AbsoluteSize.X / s
        local btnTopY = (btnAbs.Y - wrapAbs.Y) / s
        local itemH = 38; local listH = #self._options * itemH + 8
        ...
        ddList.BackgroundColor3 = theme.colors.cardBg; ddList.BorderSizePixel = 0
        ddList.ZIndex = 61; ddList.Parent = wrapper; corner(ddList, 10)
        ...
        for i, choice in ipairs(self._options) do
            ...
            item.BackgroundColor3 = theme.colors.cardBg; item.BackgroundTransparency = 1
            ...
            item.Activated:Connect(function()
                self._value = choice; ddBtn.Text = tostring(choice); closeDd()
    ...
    function self:Set(v, fire) self._value = v; ddBtn.Text = tostring(v); if fire and onChange then onChange(v) end end
    function self:Get() return self._value end
    function self:SetOptions(opts)
        self._options = opts or {}
        if not self._value or not table.find(self._options, self._value) then
            self._value = self._options[1]; ddBtn.Text = tostring(self._value or "---")
        end
    end
...
function Input.new(section, cfg)
    ...
    local theme = library.Theme; local scroll = section.tab.scroll
    local order = section:_nextOrder()
    local titleText = cfg.Name or "Input"; local descText = cfg.Description or ""
    local default = cfg.Default or ""; local numeric = cfg.Numeric and true or false
    local onConfirm = cfg.Callback; local doNotify = cfg.Notify ~= false
    local card, title, desc, cardStroke = makeCard(scroll, order, nil, theme)
    title.Text = titleText; desc.Text = descText
    local boxW = theme.sizes.controlW; local boxH = theme.sizes.controlH
    local right = theme.sizes.controlRight
    local box = Instance.new("TextBox")
    box.Size = UDim2.new(0,boxW,0,boxH); box.Position = UDim2.new(1,-(boxW+right),0.5,-boxH/2)
    box.BackgroundColor3 = theme.colors.navBg; box.Text = tostring(default)
    box.PlaceholderText = cfg.Placeholder or (numeric and "0" or "...")
    box.PlaceholderColor3 = theme.colors.textMuted; box.TextColor3 = theme.colors.textPrimary
    box.Font = Enum.Font.GothamBold; box.TextSize = 20
    box.TextXAlignment = Enum.TextXAlignment.Center; box.ClearTextOnFocus = false
    box.ZIndex = 10; box.Parent = card; corner(box, 10)
    ...
    box.Focused:Connect(function() tween(boxStroke, {Color=theme.colors.accent}, theme.anim.fast) end)
    ...
    function self:Set(v, fire) self._value = v; box.Text = tostring(v); if fire and onConfirm then onConfirm(v) end end
    function self:Get() return self._value end
...
-- KEYBIND
local Keybind = {}; Keybind.__index = Keybind
function Keybind.new(section, cfg)
    local window = section.tab.window; local library = window.library
    local theme = library.Theme; local scroll = section.tab.scroll
    local order = section:_nextOrder()
    local titleText = cfg.Name or "Keybind"; local descText = cfg.Description or ""
    local default = cfg.Default or "None"; local onChange = cfg.Callback
    local doNotify = cfg.Notify ~= false
    local mode = cfg.Mode or "Toggle"; local holdVal = false
    ...
    keyBtn.BackgroundColor3 = theme.colors.navBg; keyBtn.Text = default
    keyBtn.TextColor3 = theme.colors.textPrimary; keyBtn.Font = Enum.Font.GothamBold
    keyBtn.TextSize = 18; keyBtn.AutoButtonColor = false; keyBtn.Parent = card
    corner(keyBtn, 10); local keyStroke = stroke(keyBtn, theme.colors.cardBorder, 1.5, 0)
    local self = setmetatable({_key=default, _listening=false, _conn=nil}, Keybind)
    window:_registerComponent(self, cfg.Key or titleText)
    keyBtn.Activated:Connect(function()
        if self._listening then return end
        self._listening = true; keyBtn.Text = "Press a key..."
        tween(keyStroke, {Color=theme.colors.accent}, theme.anim.fast)
        local conn
        conn = UserInputService.InputBegan:Connect(function(input, gpe)
            if gpe then return end
            if input.UserInputType ~= Enum.UserInputType.Keyboard then return end
            self._key = input.KeyCode.Name; keyBtn.Text = self._key
            self._listening = false; tween(keyStroke, {Color=theme.colors.cardBorder}, theme.anim.fast)
    ...
    local listenerConn = UserInputService.InputBegan:Connect(function(input, gpe)
        if gpe then return end; if self._listening then return end
        if input.UserInputType ~= Enum.UserInputType.Keyboard then return end
        if self._key == "None" or self._key == "" then return end
        if input.KeyCode == Enum.KeyCode[self._key] then
            if mode == "Hold" then
                if not holdVal then holdVal = true; if onChange then onChange(true) end end
            else
                if onChange then onChange(self._key) end
            end
        end
    ...
    local listenerEnd = UserInputService.InputEnded:Connect(function(input)
        ...
        if self._key == "None" or self._key == "" then return end
        if input.KeyCode == Enum.KeyCode[self._key] then
            if holdVal then holdVal = false; if onChange then onChange(false) end end
        end
    end)
    function self:Set(key, fire) self._key = key; keyBtn.Text = tostring(key); if fire and onChange then onChange(key) end end
...
-- PARAGRAPH
local Paragraph = {}; Paragraph.__index = Paragraph
function Paragraph.new(section, cfg)
    local window = section.tab.window
    local theme = window.library.Theme
    local scroll = section.tab.scroll; local order = section:_nextOrder()
    ...
    title.Size = UDim2.new(1,-32,0,28); title.Position = UDim2.new(0,16,0,18)
    desc.Text = bodyText; desc.TextWrapped = true
    desc.Size = UDim2.new(1,-32,1,-70); desc.Position = UDim2.new(0,16,0,52)
...
function Section:_nextOrder() self._order = self._order + 1; return self.tab:_nextOrder() end
...
function Section:Destroy() if self._label and self._label.Parent then self._label:Destroy() end end
...
function Tab.new(window, cfg)
    ...
    btn.BackgroundColor3 = theme.colors.navBg; btn.Text = ""; btn.AutoButtonColor = false
    btn.ClipsDescendants = false; btn.LayoutOrder = window:_nextTabOrder()
    btn.Parent = window._navBar; corner(btn, theme.sizes.navBtnRadius)
    ...
    iconGlow.BackgroundTransparency = 1; iconGlow.Image = iconId
    iconGlow.ImageColor3 = theme.colors.navIcon; iconGlow.ImageTransparency = 0.85
    ...
    icon.ImageColor3 = theme.colors.navIcon; icon.ScaleType = Enum.ScaleType.Fit
    ...
    self._entry = {
        button=btn, stroke=btnStroke, icon=icon, iconGlow=iconGlow, hit=hit, baseSize=baseSize,
    }
    ...
    scroll.ScrollBarImageColor3 = theme.colors.accent; scroll.ScrollBarImageTransparency = 0.3
    scroll.CanvasSize = UDim2.new(0,0,0,0); scroll.AutomaticCanvasSize = Enum.AutomaticSize.Y
    scroll.ClipsDescendants = true; scroll.Visible = false; scroll.Parent = window._contentContainer
    local layout = Instance.new("UIListLayout")
    layout.FillDirection = Enum.FillDirection.Vertical; layout.Padding = UDim.new(0,theme.sizes.cardGap)
    layout.SortOrder = Enum.SortOrder.LayoutOrder; layout.HorizontalAlignment = Enum.HorizontalAlignment.Center
    layout.Parent = scroll
    local padding = Instance.new("UIPadding")
    padding.PaddingTop = UDim.new(0,6); padding.PaddingBottom = UDim.new(0,16)
...
function Tab:_nextOrder() self._order = self._order + 1; return self._order end
function Tab:CreateSection(cfg)
    local sec = Section.new(self, cfg or {}); table.insert(self._sections, sec); return sec
end
...
function Window.new(library, cfg)
    ...
    screenGui.Name = cfg.Name or "BabisUILib"; screenGui.ResetOnSpawn = false
    screenGui.IgnoreGuiInset = true; screenGui.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
    screenGui.DisplayOrder = 100; screenGui.Parent = PlayerGui
    ...
    scaleWrapper.ClipsDescendants = true; scaleWrapper.Parent = screenGui
    self._scaleWrapper = scaleWrapper

    local uiScale = Instance.new("UIScale"); uiScale.Parent = scaleWrapper; self._uiScale = uiScale

    local mainWindow = Instance.new("Frame")
    mainWindow.Size = UDim2.new(1,0,1,0); mainWindow.BackgroundColor3 = theme.colors.windowBg
    ...
    corner(header, theme.sizes.windowRadius); self._header = header

    local headerFlat = Instance.new("Frame")
    headerFlat.Size = UDim2.new(1,0,0,16); headerFlat.Position = UDim2.new(0,0,1,-16)
    headerFlat.BackgroundColor3 = theme.colors.headerTint; headerFlat.BackgroundTransparency = 1
    headerFlat.BorderSizePixel = 0; headerFlat.Parent = header; self._headerFlat = headerFlat

    local appLabel = Instance.new("TextLabel")
    appLabel.BackgroundTransparency = 1; appLabel.Text = self.title
    appLabel.TextColor3 = theme.colors.accent; appLabel.Font = theme.fonts.appName
    appLabel.TextSize = theme.textSizes.appName; appLabel.TextXAlignment = Enum.TextXAlignment.Left
    ...
    appLabel.Parent = header; self._appLabel = appLabel

    local subtitleLabel = Instance.new("TextLabel")
    subtitleLabel.BackgroundTransparency = 1; subtitleLabel.Text = self.subtitle
    ...
    subtitleLabel.Parent = header; self._subtitleLabel = subtitleLabel

    local versionLabel = Instance.new("TextLabel")
    versionLabel.BackgroundTransparency = 1; versionLabel.Text = self.version
    versionLabel.TextColor3 = theme.colors.textMuted; versionLabel.Font = theme.fonts.version
    versionLabel.TextSize = theme.textSizes.version; versionLabel.TextXAlignment = Enum.TextXAlignment.Left
    ...
    versionLabel.Parent = header; self._versionLabel = versionLabel
    ...
    infoIcon.BackgroundTransparency = 1; infoIcon.Image = library.Assets.infoIcon
    infoIcon.ImageColor3 = theme.colors.navIcon; infoIcon.ScaleType = Enum.ScaleType.Fit
    infoIcon.Parent = infoBtn; self._infoIcon = infoIcon
    ...
    minGlyph.BackgroundColor3 = theme.colors.textPrimary; minGlyph.BorderSizePixel = 0
    ...
    xBar1.BackgroundColor3 = theme.colors.textPrimary; xBar1.BorderSizePixel = 0
    ...
    xBar2.BackgroundColor3 = theme.colors.textPrimary; xBar2.BorderSizePixel = 0
    xBar2.Rotation = -45; xBar2.Parent = closeBtn; corner(xBar2, 2)

    local divider = Instance.new("Frame")
    divider.Size = UDim2.new(1,0,0,2); divider.Position = UDim2.new(0,0,0,theme.sizes.headerHeight)
    divider.BackgroundColor3 = theme.colors.divider; divider.BorderSizePixel = 0
    divider.ZIndex = 2; divider.Parent = mainWindow
    local divGrad = Instance.new("UIGradient")
    divGrad.Transparency = NumberSequence.new({
        NumberSequenceKeypoint.new(0, 0.85), NumberSequenceKeypoint.new(0.5, 0), NumberSequenceKeypoint.new(1, 0.85),
    })
    ...
    aboutTitle.BackgroundTransparency = 1; aboutTitle.Text = self.about.Title or "About"
    aboutTitle.TextColor3 = theme.colors.accent; aboutTitle.Font = Enum.Font.GothamBold
    aboutTitle.TextSize = theme.textSizes.aboutTitle; aboutTitle.TextXAlignment = Enum.TextXAlignment.Left
    ...
    aboutTitle.TextTransparency = 1; aboutTitle.Parent = infoFrame; self._aboutTitle = aboutTitle

    local aboutDesc = Instance.new("TextLabel")
    aboutDesc.BackgroundTransparency = 1; aboutDesc.Text = self.about.Body or ""
    aboutDesc.TextColor3 = theme.colors.textPrimary; aboutDesc.Font = Enum.Font.Gotham
    aboutDesc.TextSize = theme.textSizes.aboutDesc; aboutDesc.TextXAlignment = Enum.TextXAlignment.Left
    ...
    aboutDesc.Parent = infoFrame; self._aboutDesc = aboutDesc
    ...
    dIcon.BackgroundTransparency = 1; dIcon.Image = library.Assets.discordIcon
    ...
    dTitle.TextSize = theme.textSizes.discordTitle; dTitle.TextXAlignment = Enum.TextXAlignment.Left
    ...
    dSub.TextSize = theme.textSizes.discordSub; dSub.TextXAlignment = Enum.TextXAlignment.Left
    ...
    self._discordBtn = discordBtn; self._dBtnGrad = dBtnGrad
    self._dBtnStroke = dBtnStroke; self._dIcon = dIcon
    self._dTitle = dTitle; self._dSub = dSub
    ...
    local function clampToViewport(px, py, overrideH)
        ...
        local w = theme.sizes.windowWidth * s
        local h = (overrideH or theme.sizes.windowHeight) * s
...
function Window:_nextTabOrder() self._tabOrder = self._tabOrder + 1; return self._tabOrder end
...
function Window:CollectValues()
    ...
    for _, comp in ipairs(self._components) do
        if comp._configKey and comp.Get then data[comp._configKey] = comp:Get() end
    end
    return data
end
...
function Window:BuildSettingsPanel(section, opts)
    ...
    keybindCard.Size = UDim2.new(1,0,0,110); keybindCard.BackgroundColor3 = theme.colors.cardBg
    keybindCard.BorderSizePixel = 0; keybindCard.LayoutOrder = section:_nextOrder()
    keybindCard.Parent = scroll; corner(keybindCard, theme.sizes.cardRadius)
    stroke(keybindCard, theme.colors.cardBorder, 1.5, 0)

    local kbTitle = Instance.new("TextLabel")
    kbTitle.BackgroundTransparency = 1; kbTitle.Text = "Toggle Keybind"
    kbTitle.TextColor3 = theme.colors.textPrimary; kbTitle.Font = Enum.Font.GothamBold
    kbTitle.TextSize = 24; kbTitle.TextXAlignment = Enum.TextXAlignment.Left
    kbTitle.TextYAlignment = Enum.TextYAlignment.Top
    kbTitle.Size = UDim2.new(1,-160,0,28); kbTitle.Position = UDim2.new(0,16,0,24)
    kbTitle.Parent = keybindCard

    local kbDesc = Instance.new("TextLabel")
    kbDesc.BackgroundTransparency = 1; kbDesc.Text = "Key to open/close the interface"
    kbDesc.TextColor3 = theme.colors.textDesc; kbDesc.Font = Enum.Font.Gotham
    kbDesc.TextSize = 15; kbDesc.TextXAlignment = Enum.TextXAlignment.Left
    kbDesc.TextYAlignment = Enum.TextYAlignment.Top
    kbDesc.Size = UDim2.new(1,-160,0,22); kbDesc.Position = UDim2.new(0,16,0,60)
    kbDesc.Parent = keybindCard

    local keyBtn = Instance.new("TextButton")
    keyBtn.Size = UDim2.new(0,130,0,46); keyBtn.Position = UDim2.new(1,-146,0.5,-23)
    keyBtn.BackgroundColor3 = theme.colors.navBg; keyBtn.Text = opts.DefaultKeybind or "K"
    keyBtn.TextColor3 = theme.colors.textPrimary; keyBtn.Font = Enum.Font.GothamBold
    keyBtn.TextSize = 18; keyBtn.AutoButtonColor = false; keyBtn.Parent = keybindCard
    corner(keyBtn, 10); local keyStroke = stroke(keyBtn, theme.colors.cardBorder, 1.5, 0)
    ...
    keyBtn.MouseEnter:Connect(function()
        if not listening then tween(keyStroke, {Color=theme.colors.accent}, theme.anim.fast) end
    end)
    keyBtn.MouseLeave:Connect(function()
        if not listening then tween(keyStroke, {Color=theme.colors.cardBorder}, theme.anim.fast) end
    end)
    keyBtn.Activated:Connect(function()
        if listening then return end
        listening = true; keyBtn.Text = "Press a key..."
        tween(keyStroke, {Color=theme.colors.accent}, theme.anim.fast)
        listenConn = UserInputService.InputBegan:Connect(function(input, gpe)
            if gpe then return end
            if input.UserInputType ~= Enum.UserInputType.Keyboard then return end
            if input.KeyCode == Enum.KeyCode.Unknown then return end
            savedKey = input.KeyCode.Name
            stopListening()
            library:Notify("Keybind", "Set to " .. input.KeyCode.Name)
        end)
    end)
    ...
    cfgCard.BackgroundColor3 = theme.colors.cardBg; cfgCard.BorderSizePixel = 0
    cfgCard.LayoutOrder = section:_nextOrder(); cfgCard.Parent = scroll
    corner(cfgCard, theme.sizes.cardRadius); stroke(cfgCard, theme.colors.cardBorder, 1.5, 0)

    local cfgIcon = Instance.new("ImageLabel")
    cfgIcon.Size = UDim2.new(0,26,0,26); cfgIcon.Position = UDim2.new(0,14,0,14)
    cfgIcon.BackgroundTransparency = 1; cfgIcon.Image = library.Assets.cfgIcon
    cfgIcon.ImageColor3 = theme.colors.cfgIcon; cfgIcon.ScaleType = Enum.ScaleType.Fit
    cfgIcon.Parent = cfgCard

    local cfgHeader = Instance.new("TextLabel")
    cfgHeader.Size = UDim2.new(1,-60,0,26); cfgHeader.Position = UDim2.new(0,48,0,16)
    cfgHeader.BackgroundTransparency = 1; cfgHeader.Text = "Configuration"
    cfgHeader.TextColor3 = theme.colors.textPrimary; cfgHeader.Font = Enum.Font.GothamBold
    cfgHeader.TextSize = theme.textSizes.cfgTitle; cfgHeader.TextXAlignment = Enum.TextXAlignment.Left
    ...
    local function smallLabel(y, text)
        ...
        lbl.TextColor3 = theme.colors.textPrimary; lbl.Font = Enum.Font.GothamBold
        lbl.TextSize = theme.textSizes.cfgLabel; lbl.TextXAlignment = Enum.TextXAlignment.Left
        lbl.Parent = cfgCard
        return lbl
    end
    ...
    nameBox.BackgroundColor3 = theme.colors.cfgBtnBg; nameBox.Text = ""
    nameBox.PlaceholderText = "type a name..."; nameBox.PlaceholderColor3 = theme.colors.textMuted
    nameBox.TextColor3 = theme.colors.textPrimary; nameBox.Font = Enum.Font.GothamBold
    nameBox.TextSize = theme.textSizes.cfgInput; nameBox.TextXAlignment = Enum.TextXAlignment.Left
    nameBox.ClearTextOnFocus = false; nameBox.Parent = cfgCard
    corner(nameBox, 8); local nameStroke = stroke(nameBox, theme.colors.cfgBtnBorder, 1.2, 0)
    local namePad = Instance.new("UIPadding"); namePad.PaddingLeft = UDim.new(0,12); namePad.Parent = nameBox
    ...
    local function smallButton(y, text, onClick)
        ...
        btn.BackgroundColor3 = theme.colors.cfgBtnBg; btn.Text = text
        btn.TextColor3 = theme.colors.textPrimary; btn.Font = Enum.Font.GothamBold
        btn.TextSize = theme.textSizes.cfgBtn; btn.AutoButtonColor = false
        btn.Parent = cfgCard; corner(btn, 8)
        ...
        btn.Activated:Connect(function()
            ...
            btn.Size = UDim2.new(1,-32,0,32); btn.Position = UDim2.new(0,16,0,y+1)
            task.delay(0.07, function() btn.Size = osz; btn.Position = opos end)
            if onClick then onClick() end
        end)
        return btn
    end
    ...
    ddBtn.BackgroundColor3 = theme.colors.cfgBtnBg; ddBtn.Text = "---"
    ddBtn.TextColor3 = theme.colors.textPrimary; ddBtn.Font = Enum.Font.GothamBold
    ddBtn.TextSize = theme.textSizes.cfgInput; ddBtn.TextXAlignment = Enum.TextXAlignment.Left
    ddBtn.AutoButtonColor = false; ddBtn.Parent = cfgCard
    corner(ddBtn, 8); local ddStroke = stroke(ddBtn, theme.colors.cfgBtnBorder, 1.2, 0)
    ...
    arrow.TextColor3 = theme.colors.textDesc; arrow.Font = Enum.Font.GothamBold
    ...
    local function openDd()
        ...
        ddBackdrop.Size = UDim2.new(1,0,1,0); ddBackdrop.BackgroundTransparency = 1
        ddBackdrop.Text = ""; ddBackdrop.AutoButtonColor = false; ddBackdrop.ZIndex = 60
        ddBackdrop.Parent = self._scaleWrapper; ddBackdrop.Activated:Connect(closeDd)
        ...
        local btnTopY = (btnAbs.Y - wrapAbs.Y) / s
        local itemH = 34; local listH = #list * itemH + 8
        local relY = btnTopY - listH - 6
        ddList = Instance.new("Frame")
        ddList.Size = UDim2.new(0,relW,0,listH); ddList.Position = UDim2.new(0,relX,0,relY)
        ddList.BackgroundColor3 = theme.colors.cardBg; ddList.BorderSizePixel = 0
        ddList.ZIndex = 61; ddList.Parent = self._scaleWrapper
        corner(ddList, 10); stroke(ddList, theme.colors.accent, 1.5, 0)
        local pad = Instance.new("UIPadding")
        pad.PaddingTop = UDim.new(0,4); pad.PaddingBottom = UDim.new(0,4); pad.Parent = ddList
        local lay = Instance.new("UIListLayout"); lay.FillDirection = Enum.FillDirection.Vertical; lay.Parent = ddList
        for i, name in ipairs(list) do
            ...
            item.MouseLeave:Connect(function() tween(item, {BackgroundTransparency=1}, theme.anim.fast) end)
            item.Activated:Connect(function()
                selectedName = name; ddBtn.Text = name; closeDd()
            end)
        end
        tween(ddStroke, {Color=theme.colors.accent}, theme.anim.fast)
    end
    ...
    autoloadText.TextColor3 = theme.colors.textDesc; autoloadText.Font = Enum.Font.GothamBold
